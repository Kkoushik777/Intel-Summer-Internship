...
bool CryptoWrapper::getDhSharedSecret(INOUT DhContext* dhContext, IN const BYTE* peerPublicKey, IN size_t peerPublicKeySizeBytes, OUT BYTE* sharedSecretBuffer, IN size_t sharedSecretBufferSizeBytes)
{
	...
	if (derivationCtx == NULL) {
		printf("Error in creating derivation context with dhContext at getDhSharedSecret \n");
		goto err;
	}
	...
	if (rc != 1) {
		printf("Error in initiating derivation context at getDhSharedSecret \n");
		goto err;
	}
	...
	if (rc != 1) {
		printf("Error in setting peer at getDhSharedSecret \n");
		goto err;
	}
	...
	if (rc != 1) {
		printf("Error in derive at getDhSharedSecret \n");
		goto err;
	}
